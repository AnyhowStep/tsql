...
sqlite3_result_int64 = Module['cwrap']('sqlite3_result_int64', '', ['number', 'number', 'number']);
...
if (ENVIRONMENT_IS_NODE) {
  ...
  quit_ = function(status) {
    console.log("quit triggered with status", status, new Error());
    //process['exit'](status);
  };
...
if (ENVIRONMENT_IS_SHELL) {
  ...
  if (typeof quit === 'function') {
    quit_ = function(status) {
      console.log("quit caled with status", status, new Error());
      quit(status);
    };
  }
...
var wasmBinaryFile = 'sql-wasm.wasm';

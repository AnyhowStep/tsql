...
tape(__filename, async (t) => {
    ...
    await pool.acquire(async (connection) => {
        ...
        t.deepEqual(
            result.warnings[0],
            {
                type : tsql.SchemaValidationWarningType.COLUMN_GENERATED_ON_APPLICATION_ONLY_INSERT_DISABLED,
                description : `Column "test"."testVal" is generated on application only; INSERTs will fail but INSERTs are disabled`,
                tableAlias : "test",
                columnAlias : "testVal",
            }
        );

...
export const test : Test = ({tape, pool}) => {
    tape(__filename, async (t) => {
        await pool.acquire(async (connection) => {
            for (let i=0; i<10; ++i) {
                /**
                 * Make `now0` accurate to 0.01-second
                 */
                const now0 = new Date(Math.floor(new Date().getTime()/10) * 10);

                await tsql
                    .selectValue(() => tsql.currentTimestamp2())
                    ...
                    .then((value) => {
                        ...
                        t.deepEqual(value.getUTCMilliseconds()%10, 0, `${value.toISOString()} should be accurate to 0.01-second`);
                    })
                    .catch((err) => {
                        t.fail(err.message);
                    });

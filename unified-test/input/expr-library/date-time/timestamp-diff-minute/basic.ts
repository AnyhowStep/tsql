                            ...
                            for (let minute=0; minute<=59; minute+=17) {
                                for (let second=0; second<=59; second+=13) {
                                    for (let millisecond=0; millisecond<=999; millisecond+=173) {
                                        ...
                                        await tsql
                                            .selectValue(() => tsql.timestampDiffMinute(
                                                from,
                                                to
                                            ))
                                            .fetchValue(connection)
                                            .then((value) => {
                                                const expected = Math.trunc((to.getTime()-from.getTime())/60/1000);
                                                t.deepEqual(value, BigInt(expected), `TIMESTAMPDIFF(MINUTE, ${from.toISOString()}, ${to.toISOString()}) = ${expected}`);
                                            })
                                            .catch((err) => {
                                                t.fail(String(err));
                                            });

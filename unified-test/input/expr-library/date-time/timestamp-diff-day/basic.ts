                ...
                for (let month=1; month<=12; ++month) {
                    ...
                    for (let day=26; day<=lastDayOfMonth; ++day) {
                        for (let hour=0; hour<=23; ++hour) {
                            ...
                            await tsql
                                .selectValue(() => tsql.timestampDiffDay(
                                    from,
                                    to
                                ))
                                .fetchValue(connection)
                                .then((value) => {
                                    const expected = Math.trunc((to.getTime()-from.getTime())/24/60/60/1000);
                                    t.deepEqual(value, BigInt(expected), `TIMESTAMPDIFF(DAY, ${from.toISOString()}, ${to.toISOString()}) = ${expected}`);
                                })
                                .catch((err) => {
                                    t.fail(String(err));
                                });

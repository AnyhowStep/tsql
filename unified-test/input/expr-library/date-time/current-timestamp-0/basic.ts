...
export const test : Test = ({tape, pool}) => {
    tape(__filename, async (t) => {
        await pool.acquire(async (connection) => {
            for (let i=0; i<10; ++i) {
                /**
                 * Make `now0` accurate to 1-second
                 */
                const now0 = new Date(Math.floor(new Date().getTime()/1000) * 1000);

                await tsql
                    .selectValue(() => tsql.currentTimestamp0())
                    ...
                    .then((value) => {
                        ...
                        t.deepEqual(value.getUTCMilliseconds(), 0, `${value.toISOString()} should be accurate to 1-second`);
                    })
                    .catch((err) => {
                        t.fail(err.message);
                    });
